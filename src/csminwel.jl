...
function Optim.optimize(f::Function, g!::Function, initial_x::Array, method::Csminwel, options::OptimizationOptions)
  opts = Array{Any}(0)
  for name in fieldnames(options)
    push!(opts, (name, getfield(options, name)))
  end
  csminwel(f, g!, initial_x; opts...)
end
...
function csminwel(fcn::Function,
                  ...
                  verbose::Symbol      = :none,
                  rng::AbstractRNG     = MersenneTwister(), kwargs...)
...
function csminwel(fcn::Function,
    ...
    grad{T<:Number}(x::Array{T}) = csminwell_grad(fcn, x)
    csminwel(fcn, grad, x0, H0;
